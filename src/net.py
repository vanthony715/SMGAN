...
class Generator(nn.Module):
    ...
    def forward(self, z: torch.tensor) -> torch.tensor:
        ...
        '''
        z = z.long() #embedding expects long()

        ##embedding
        embedded = self.embedding(z)  #shape: (batch_size, max_length, embedding_dim)
        if embedded.dim() == 4:
            embedded = embedded.view(embedded.shape[0]*z.shape[1], z.shape[2], -1)  #check extra dim and resize accordingly

        ##recurrence
        gru_out, _ = self.gru(embedded)  #shape: (batch_size, max_length, hidden_dim)
        gru_out = self.dropout(gru_out)
        
        ##multi-head attention (expects (seq_len, batch_size, hidden_dim))
        ...
        output_indices = torch.argmax(output, dim=-1)  #convert from continuous to discrete indices for discriminator embedding layer
...
class Discriminator(nn.Module):
    ...
    def forward(self, x: torch.tensor) -> torch.tensor:
        ...
        Builds graph and forwards
        '''
        x = x.long() #embedding expects long()
        
        ##embedding
        embedded = self.embedding(x)  #shape: (batch_size, max_length, embedding_dim)
        if embedded.dim() == 4:
            embedded = embedded.view(embedded.shape[0]*x.shape[1], x.shape[2], -1)  #check extra dim and resize accordingly
